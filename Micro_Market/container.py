...
class Container(tk.Frame):
    ...
    def create_interface(self):
        ...
        # Panel izquierdo
        left_frame = tk.Frame(self, bg="light gray", highlightbackground="black", highlightthickness=1)
        ...
        # Panel derecho
        right_frame = tk.Frame(self, bg="light gray", highlightbackground="black", highlightthickness=1)
        ...
        self.add_button(right_frame, "Categoria", self.access_configuracion, 10, 90, 180, 60)
    ...
    def access_ventas(self):
        if self.user_role in ["Administrador", "Vendedor"]:
            ventas_frame = Ventas(self.controlador, self.db_connection, self.username, self.user_role)
    ...
    def access_reportes(self):
        if self.user_role == "Administrador":
            reportes_frame= Reportes(self.controlador,self.db_connection)
            reportes_frame.place(x=0,y=0,width=1100, height=650)
            reportes_frame.tkraise()
        else:
            self.show_access_denied()
    ...
    def access_usuarios(self):
        if self.user_role == "Administrador":
            usuario_frame=Usuarios(self.controlador, self.db_connection, self.username)
